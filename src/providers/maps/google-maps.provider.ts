...
export class GoogleMapsProvider extends BaseMapsProvider {
  ...
  async searchLocation(query: string): Promise<LocationDetails> {
    ...
    if (!this.apiKey) {
      this.logger.warn('GOOGLE_MAPS_API_KEY is not defined! Falling back to simulated GPS mapping...');
    ...
    try {
      ...
      if (response.data.status !== 'OK') {
        throw new Error(`Google Maps API error status: ${response.data.status}`);
      }
      ...
      this.logger.error(`Google Maps geocoding failure for "${query}":`, error.message);
  ...
  private getMockLocation(query: string): LocationDetails {
    ...
    if (normalized.includes('tokyo')) {
      return { formattedAddress: 'Tokyo, Japan', latitude: 35.6762, longitude: 139.6503, placeId: 'chij5z9at-tlggars1r6dpl2tfa' };
    }
    if (normalized.includes('paris')) {
      return { formattedAddress: 'Paris, France', latitude: 48.8566, longitude: 2.3522, placeId: 'chij0w82t_jxbkgrzqy3vy7t9f0' };
    }
    if (normalized.includes('london')) {
      return { formattedAddress: 'London, UK', latitude: 51.5074, longitude: -0.1278, placeId: 'chijdd4hrwobudgr-3w_vgcxny8' };
    }
