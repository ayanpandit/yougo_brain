...
export class ValidationService {
  ...
  parseAndValidate<T>(rawContent: string, schema: z.ZodSchema<T>): T {
    ...
    try {
      ...
      this.logger.warn(`Standard JSON parsing failed: ${firstParseError.message}. Attempting regex repair...`);
      ...
      try {
        ...
        this.logger.error('JSON parsing failed completely even after syntax repair.');
        throw new Error(`JSON Syntax Error: ${secondParseError.message}. Content was: ${cleaned}`);
      }
    }
  ...
  private cleanAndExtractJson(content: string): string {
    ...
    if (firstBrace === -1 || lastBrace === -1 || lastBrace < firstBrace) {
      this.logger.warn('Could not detect braces in the LLM string. Processing raw content.');
      return trimmed;
    }
